...
class DockerDiagnosticRequest(BaseModel):
    ...
    docker_inspect:   str | None = Field(default=None, description="Saída de `docker inspect <container>`")
...
class HardwareDiagnosticRequest(BaseModel):
    ...
    problem_description: str = Field(..., min_length=10)
    hw_output:       str | None = Field(default=None, description="Saída de lshw, lspci, dmesg, journalctl")
